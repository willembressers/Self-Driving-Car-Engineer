...
float motion_model(float pseudo_position, float movement, vector<float> priors,
  ...
  // loop over state space for all possible positions
  for (float x=0; x< map_size; ++x) {
    
    // convert to float
    float next_pseudo_position = x;

    // calculate: the distance from (pseudo_position to x)
    float distance = pseudo_position - next_pseudo_position;

    // calculate: the transition probabilities
    float transition_prob = Helpers::normpdf(distance, movement, 
                                             control_stdev);

    // estimate: the probability for the motion model
    position_prob += transition_prob * priors[x];
  }
